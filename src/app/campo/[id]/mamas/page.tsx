...
import { CalendarDays, AlertTriangle, UtensilsCrossed, BookOpen } from 'lucide-react'
...
export default async function MamasPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: campo }, { count: restricoesCount }] = await Promise.all([
  ...
  const modulos = [
    { href: 'ementa',    label: 'Plano de Refeições', icon: CalendarDays,    desc: 'Planear refeições dia a dia',    color: 'text-green-700 bg-green-50 border-green-200' },
    ...
    <main className="min-h-screen">
      <div className="bg-[#2D5016] text-white px-4 pt-10 pb-6">
        <div className="max-w-lg mx-auto">
          ...
          <p className="text-green-200 text-sm mt-1">O teu espaço de planeamento 🍳</p>
        </div>
      </div>
