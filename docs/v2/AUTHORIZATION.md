...
## Estado real medido (fecho da Fase 2) — o que as secções abaixo descrevem é o modelo-alvo; isto é o que está confirmado por teste automatizado hoje

A maior parte deste documento (Personas, Policies conceptuais, Fronteira de confiança) descreve o modelo **alvo**, escrito na Fase 1 com naming futuro (`expenses`/`refunds`/`camps`). Nem tudo está implementado ainda ao nível descrito. Três achados reais, confirmados por `tests/security/role-matrix.test.ts` (59/59) e `tests/security/storage-matrix.test.ts` (14/14) contra produção:

1. **`adjunto` (via `camp_membership` real) pode ler/criar/editar no seu campo, mas NÃO apagar.** `despesas_delete` (migration 042) é `is_admin() OR is_treasurer() OR has_legacy_anon_access(campo_id)` — de propósito sem `can_edit_camp()`. Hoje, um Adjunto real só apaga através da exceção legacy (é como o "Eliminar despesa" da UI funciona nos 11 campos reais); um Adjunto autenticado por `camp_membership`, num campo já fechado, não conseguiria. Não é uma regressão — é uma decisão já tomada na migration 042, agora documentada e coberta por teste.
2. **As páginas de leitura de `/campo/[id]/adjuntos/*` (`page.tsx`) usam `src/lib/supabase/server.ts` — um cliente anon simples, NÃO consciente de cookies/sessão.** Isto significa que a leitura destas páginas depende inteiramente de `legacy_anon_access`; uma sessão Supabase Auth real (`camp_membership`) não tem qualquer efeito na leitura destas páginas específicas hoje — só nas Server Actions de escrita (`createSessionServerClient()`), que já são conscientes de sessão. É um gap real de arquitetura, não um bug de RLS: a "Fronteira de confiança" descrita mais abaixo já é verdade para escrita, ainda não para leitura nestas rotas. Fica identificado para uma iteração futura (fora do âmbito do fecho desta fase).
3. **Storage (`faturas`) não tem qualquer segmentação por caminho/campo — confirmado por introspecção direta (`pg_policies`), não só inferido do flag `public`.** As três policies (`faturas_public_read/insert/delete`) são `bucket_id = 'faturas'` para `{public}`, sem predicado de caminho — qualquer role (incl. `anon`) lê/escreve/apaga em qualquer pasta de qualquer campo. Consequência direta e já aceite da decisão "Opção A" (subfase 2.5); `tests/security/storage-matrix.test.ts` documenta isto com `it.fails(...)` (mesmo padrão de `anon-read-baseline.test.ts`) para nunca ficar silenciosamente esquecido.
4. **`/admin/memberships` é a exceção positiva:** já é session-aware (`createSessionServerClient()`) e verifica `profiles.global_role === 'admin'` explicitamente no servidor antes de mostrar qualquer conteúdo — confirmado por `tests/e2e/admin-memberships.spec.ts` com sessões reais injectadas (admin ALLOW; viewer/treasurer/field_viewer DENY com o motivo certo).
