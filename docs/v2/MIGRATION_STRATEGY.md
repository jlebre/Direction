...
**Estado: FECHADA — incluindo a subfase 2.8 (leituras session-aware + isolamento de Storage), pedida explicitamente antes de avançar para a Fase 3.** Auth, RLS core, Server Boundary de escrita, Danger Zone, harness de teste completo (109/109 Vitest + 25/25 Playwright, typecheck/build limpos), leituras session-aware e Storage isolado por campo — todos feitos e verificados contra produção. Único ponto que continua dependente de uma condição operacional (não de código) por resolver: **`legacy_anon_access` continua necessário para os 11 campos reais**, porque nenhum tem ainda uma `camp_membership` real populada (rollout manual, ver "Migrate memberships" abaixo) — ver "Remaining Security Risks" no relatório de fecho para o detalhe exacto.
