...
**Estado: FECHADA quanto ao âmbito prometido (Auth, RLS core, Server Boundary de escrita, Danger Zone, harness de teste completo — 106/106 Vitest + 18/18 Playwright, typecheck/build limpos). Duas exceções conhecidas, já aceites por decisão explícita anterior ou fora do âmbito prometido, ficam documentadas e não bloqueiam o fecho — ver "Remaining Security Risks" no relatório de fecho:**
1. Storage (`faturas`) não segmenta por campo em nenhuma operação (decisão "Opção A", subfase 2.5 — não uma regressão desta ronda).
2. As páginas de leitura de `/campo/[id]/adjuntos/*` usam um cliente anon não consciente de sessão — a "fronteira de servidor" da subfase 2.6 cobre ESCRITAS (Server Actions), que era o âmbito combinado; LEITURA autenticada por `camp_membership` nessas páginas específicas fica para uma iteração futura.
...
- **Regressão (2.7) — feita.** `npm run typecheck` (0 erros), `npx vitest run` (106/106), `npm run test:e2e` (18/18, contra a app deployada), `npm run test:e2e:cleanup` (0 resíduos), `npm run build` (limpo). `npm run backup` corrido antes da migration 043 e no fecho — `PASS` nas duas vezes, contagens de BD/Storage idênticas.
- **Rollback:** SQL documentado em `supabase/rollbacks/` (041, 042, 043) — restaura literalmente as policies `USING(true)` originais (041/042) ou remove só a função RPC sem tocar em dados (043).
