...
- **Switch writes (2.6, a seguir) — não feito ainda.** Todas as escritas financeiras (`NovaDespesaClient` e equivalentes) continuam a usar o cliente anon simples (`@/lib/supabase/client`), não o cliente de sessão — funciona hoje só porque todos os campos reais têm a exceção legacy ativa. Isto é agora um PRÉ-REQUISITO real (não só boa prática) para poder fechar qualquer campo: sem migrar estes componentes para o cliente de sessão, um adjunto autenticado de um campo fechado não conseguiria escrever mesmo tendo membership.
